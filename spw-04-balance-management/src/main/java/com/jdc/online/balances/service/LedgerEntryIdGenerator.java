...
public class LedgerEntryIdGenerator {
	...
	public LedgerEntryPk next(long memberId, LocalDate issuedAt) {
		
		var seqId = new LedgerEntrySeqPk();
		seqId.setMemberId(memberId);
		seqId.setIssuedAt(issuedAt);
		
		var seqEntity = repo.findById(seqId).orElseGet(() -> {
			var seq = new LedgerEntrySeq();
			seq.setId(seqId);
			return repo.save(seq);
		});
		
		return seqEntity.next();
	}
}
